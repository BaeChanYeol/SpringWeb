...
<body>
	...
	<script>
		...
		$(function() {
			...
			//등록을 담당하는 함수
			function regist() {
				//세션에서 현재 로그인 중인 아이디를 얻어오자.
				const user_id = '${login.userId}';
				//자바 스크립트 파일 확장자 체크 검색
				let file = $('#file').val();
				...
				//data[index] -> 파일 업로드 버튼이 여러개 존재할 수도 있어요
				//file[index] -> 파일이 여러개 전송할 수 있기 때문에, 몇번째 파일인지

				//폼 데이터 객체에 사용자가 업로드한 파일의 정보들이 들어있는 객체를 전달
				formData.append("file", data[0].files[0]);
				//content(글 내용) 값을 얻어 와서 폼데이터에 추가
				...
				$.getJSON(
						...
						function(list) {
							...
							for(let i=0; i<list.length; i++){
			                     ...
			                     str += "<a href='"+ list[i].bno +"' id='delBtn'><i class='glyphicon glyphicon-remove'></i>삭제하기</a>";
			                     str += "</div>";
			                     $("#contentDiv").html(str);
							}
						}
			...
			//일치하지 않으면 문자열 "noAuth"를 리턴, 성공하면 Success 를 리턴
			
			$('#contentDiv').on('click', '#delBtn', function(e) {
				e.preventDefault();
		...
		$(window).scroll(function() {
			//윈도우 (디바이스)의 높이와 현재 스크롤 위치 값을 더한뒤, 문서(컨턴츠) 높이와 비교해서 같다면 로직을 수행
			//문서 높이 - 브라우처 창 = 스크롤창의 끝 높이와 같다면 -> 새로운 내용을 불러오기
			// scrollTop 은 스크롤 현재위치
			if($(window).scrollTop() == $(document).height() - $(window).height()){
				console.log(page++);
				$('#contentDiv').append("<h1> page : " + page + "</h1>");
				$('#contentDiv').append("<br>무한</br><br>페이징</br><br>로드</br><br>중입니다.</br>"+"<br>무한</br><br>페이징</br><br>로드</br><br>중입니다.</br>"+"<br>무한</br><br>페이징</br><br>로드</br><br>중입니다.</br>");
